document.addEventListener('DOMContentLoaded', function() {
    ...
    if (contactForm) {
        ...
        function validateField(input) {
            ...
            const name = input.name || input.getAttribute('data-type');

            if (input.hasAttribute('required') && !value) {
                showError(input, errorMessages[name] || 'This field is required');
                return false;
            }
    ...
    if (animatedElements.length > 0 && !document.getElementById('scroll-animations-style')) {
...
/* ZAPPY_MOTION_RUNTIME (V2 designer mode) */
(function(){
  if (typeof window === 'undefined' || typeof document === 'undefined') return;
  var reduce = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  if (reduce) {
    // Reveal elements should not be hidden if motion is disabled
      ...
      entries.forEach(function(entry){
        ...
        if (el.hasAttribute('data-reveal-stagger')) {
          ...
          for (var i = 0; i < children.length; i++) {
            (function(child, idx){
              setTimeout(function(){ child.classList.add('is-revealed'); },
                idx * (parseInt(getComputedStyle(el).getPropertyValue('--stagger-step')) || 70));
            })(children[i], i);
          }
        } else {
          el.classList.add('is-revealed');
  ...
  function setupMarquee(){
    document.querySelectorAll('[data-marquee]').forEach(function(el){
      ...
      // Defensive de-dup: if the section AI already emitted more than one
      // marquee-track sibling (or pre-duplicated items inside one track for
      // a "seamless loop"), strip the extras so we render exactly TWO tracks
      // — the original + one runtime-cloned copy. Three+ tracks wrap to new
      // visual rows and produce the "duplicated marquee strip" bug.
      for (var t = 1; t < tracks.length; t++) { tracks[t].parentNode.removeChild(tracks[t]); }
      var track = tracks[0];
      if (track.children.length === 0) return;
      var speed = parseFloat(el.getAttribute('data-marquee')) || 40; // seconds for one full cycle
      var clone = track.cloneNode(true);
      clone.setAttribute('aria-hidden', 'true');
      el.appendChild(clone);
      track.style.animation = 'zappy-marquee ' + speed + 's linear infinite';
      clone.style.animation = 'zappy-marquee ' + speed + 's linear infinite';
    });
  }
  ...
  function setupCountUp(){
    ...
    var io = new IntersectionObserver(function(entries){
      entries.forEach(function(entry){
        ...
        var duration = parseInt(el.getAttribute('data-count-duration')) || 1400;
    ...
    document.querySelectorAll('[data-count-up]').forEach(function(el){ io.observe(el); });
  }
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '2c91f936-7d3d-493a-b9bc-64bce39c6455',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '2c91f936-7d3d-493a-b9bc-64bce39c6455';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '2c91f936-7d3d-493a-b9bc-64bce39c6455';
